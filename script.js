...
items.style.display=open?'grid':'none';

menu.items.filter(x=>x.category===cat).forEach(item=>{
let card=document.createElement('div');
card.className='item-card';
card.style.background=getColor(i);
...
function getColor(i){
let c=['#e74c3c','#3498db','#27ae60','#9b59b6','#f39c12'];
return c[i%c.length];
}
...
document.getElementById('delivery-fee-section').style.display=t==='delivery'?'block':'none';
updateTotal();
}
...
let html=`
<div style="width:72mm;font-family:Tahoma;font-size:12px;direction:rtl;text-align:right;">
...
if(i.note)html+=`<small>ملاحظة: ${i.note}</small><br>`;
});

html+=`
<hr>
<b>الإجمالي: ${document.getElementById('grandTotal').textContent}</b>
<hr>
<div style="text-align:center;">
...
itemsText+=`▫️ ${i.name} × ${i.qty} = ${total} ر.س\n`;
if(i.note) itemsText+=`   ✏️ ${i.note}\n`;
});

const total=document.getElementById('grandTotal').textContent;

const appLink="https://deerty666.github.io/menu.html?branch=branch1";

const message=`🧾 *طلب جديد - سحايب ديرتي*
...
let name=prompt("اسم جديد",item.name);
let price=parseFloat(prompt("سعر جديد",item.price));
